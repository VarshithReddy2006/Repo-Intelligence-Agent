...
def main():
    ...
    repo_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "benchmark_temp_repo")
    )

    # Setup custom registry and pipeline
    benchmark_registry = AnalysisRegistry()
    benchmark_registry.register(
        "Symbol Index", SymbolService, dependencies=[], outputs=["symbols"]
    )
    benchmark_registry.register(
        "Dependency Graph",
        ArchitectureService,
    ...
    try:
        ...
        metrics = {"sequential": defaultdict(list), "parallel": defaultdict(list)}
        ...
        report.append(
            "| Scenario | Sequential Wall-clock (ms) | Parallel Wall-clock (ms) | Speedup Factor | CPU Time (ms) | Executed Tasks | Skipped Tasks | Cache Hit Rate |"
        )
        report.append("| --- | --- | --- | --- | --- | --- | --- | --- |")
        ...
        report.append(
            "| Scenario | Parallel Duration (ms) | Total Task Time (ms) | Worker Utilization | Parallel Efficiency |"
        )
        report.append("| --- | --- | --- | --- | --- |")
        ...
        for name, _ in scenarios:
            ...
            utilization = (
                total_task_time / (par["wall"] * 4) if par["wall"] > 0 else 0.0
            )

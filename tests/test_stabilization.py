...
from services.github_service import GitHubService, BranchNotFoundError, RepositoryNotFoundError
...
def test_clone_fallback_authentication_failure():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert "Authentication failure" in str(exc_info.value) or "Repository private" in str(exc_info.value)
...
def test_clone_branch_auto_discovery():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.side_effect = [
            ...
            MagicMock(returncode=0, stdout="ref: refs/heads/master\tHEAD\n"), # branch discovery
...
def test_sqlite_embedding_cache_and_deduplication():
    ...
    with patch("services.embedding_service._get_model", return_value=mock_model), \
         patch("services.embedding_service._get_cached_embedding", return_value=None) as mock_get_cache, \
         patch("services.embedding_service._save_embeddings_to_cache_bulk") as mock_save_cache:
...
def test_startup_warmup_execution():
    ...
    with patch("services.embedding_service._get_model") as mock_embed_model, \
         ...
         mock_transformer.encode.assert_called_with(["Represent this sentence: dummy text"], show_progress_bar=False)

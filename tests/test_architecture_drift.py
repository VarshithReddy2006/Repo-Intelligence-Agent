...
class TestArchitectureDriftAlgorithms(unittest.TestCase):
    ...
    def test_is_entry_point_file(self):
        ...
        # FastAPI import
        self.assertTrue(self.service._is_entry_point_file("some_module.py", ["fastapi"]))
        self.assertFalse(self.service._is_entry_point_file("some_module.py", ["requests"]))
    ...
    def test_compute_drift_risk(self):
        ...
        score, level = self.service._compute_drift_risk(
            new_cycles=[], changed_core_files=[], new_entry_points=[],
            coupling_increase=[], blast_radius_level="LOW",
            added_dependencies=[], removed_entry_points=[]
        )
        ...
        # 2. Cycles added -> 45
        score, level = self.service._compute_drift_risk(
            new_cycles=[["A", "B"]], changed_core_files=[], new_entry_points=[],
            coupling_increase=[], blast_radius_level="LOW",
            added_dependencies=[], removed_entry_points=[]
        )
        ...
        score, level = self.service._compute_drift_risk(
            new_cycles=[], changed_core_files=["core.py"], new_entry_points=["main.py"],
            coupling_increase=[], blast_radius_level="LOW",
            added_dependencies=[], removed_entry_points=[]
        )
        ...
        score, level = self.service._compute_drift_risk(
            new_cycles=[["A", "B"]], changed_core_files=["core.py"], new_entry_points=["main.py"],
            coupling_increase=[CouplingChange(file="core.py", before=5, after=12)], blast_radius_level="HIGH",
            added_dependencies=[DependencyEdge(source="A", target="B")], removed_entry_points=[]
        )
    ...
    def test_compute_drift_categories(self):
        ...
        cats = self.service._compute_drift_categories(
            new_cycles=[["A", "B"]], resolved_cycles=[["C", "D"]],
            coupling_increase=[CouplingChange(file="A", before=2, after=8)],
            coupling_decrease=[], new_entry_points=["main.py"],
            removed_entry_points=[], added_dependencies=[], removed_dependencies=[]
        )
    ...
    def test_generate_top_findings(self):
        ...
        self.assertEqual(findings[2], "Coupling increased significantly in core.py (from 2 to 8)")
    ...
    def test_analyze_drift_success(self, mock_ts, mock_arch, mock_graph, mock_github):
        ...
        mock_github_instance.fetch_pull_request_files.return_value = [
            {"filename": "services/architecture_service.py", "status": "modified", "additions": 30, "deletions": 5, "changes": 35}
        ]
        # Content fetch mock
        ...
        self.assertEqual(res.added_dependencies[0].source, "services/architecture_service.py")
...
class TestPRDriftEndpoints(unittest.TestCase):
    ...
    def test_endpoint_drift_analyze_success(self, mock_drift):
        ...
        response = client.post("/api/architecture/drift", json={"pr_url": "https://github.com/owner/repo/pull/42"})

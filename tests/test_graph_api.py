...
def setup_test_repo():
    ...
    files = [
        {
            "path": "backend/api.py",
            "content": "import services.auth_service\nimport services.retrieval_service",
        },
    ...
    arch_service.build(repo_name=repo_name, files=files, force_rebuild=True)
...
def test_graph_api_limits(setup_test_repo) -> None:
    ...
    graph_data = graph_service.get_visualization_graph(
        repo_name=repo_name, architecture_service=arch_service, max_nodes=2, max_edges=5
    )

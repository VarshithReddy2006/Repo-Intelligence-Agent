...
def registry():
    ...
    reg.register("Symbol Index", type(None), dependencies=[], outputs=["symbols"], schema_version=1)
    reg.register("Dependency Graph", type(None), dependencies=["Symbol Index"], outputs=["graphs/dependency"], schema_version=1)
    reg.register("Call Graph", type(None), dependencies=["Symbol Index", "Dependency Graph"], outputs=["graphs/call"], schema_version=1)
    return reg
...
def test_plan_first_build(registry):
    ...
    change_set = ChangeSet(
        added={"main.py"}, modified=set(), deleted=set(), renamed={}, unchanged=set(), repository_changed=True
    )
...
def test_plan_no_change_skip(registry):
    ...
    change_set = ChangeSet(
        added=set(), modified=set(), deleted=set(), renamed={}, unchanged={"main.py"}, repository_changed=False
    )
    # Mock snapshots exist
    store = MockSnapshotStore({"symbols", "graphs/call"})
    graphs = MockGraphService({"test/repo", "test/repo_call_graph"})  # Both graphs exist
...
def test_plan_code_change_partial(registry):
    ...
    change_set = ChangeSet(
        added=set(), modified={"main.py"}, deleted=set(), renamed={}, unchanged=set(), repository_changed=True
    )
    # Mock snapshots exist
    store = MockSnapshotStore({"symbols", "graphs/call"})

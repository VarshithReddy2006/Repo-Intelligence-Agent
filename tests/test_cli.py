...
def test_cli_health(respx_mock=None):
    ...
    with patch("httpx.get", return_value=mock_response) as mock_get:
        ...
        mock_get.assert_called_once_with("http://test-server/api/v1/health", timeout=10.0)
...
def test_cli_graph():
    ...
    with patch("httpx.get", return_value=mock_response) as mock_get:
        result = runner.invoke(app, ["graph", "owner", "repo", "--url", "http://test-server"])
        assert result.exit_code == 0
        assert "nodes" in result.stdout
        mock_get.assert_called_once_with("http://test-server/api/v1/architecture/owner/repo/graph", timeout=30.0)

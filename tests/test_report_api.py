...
def test_cli_report_command(tmp_path):
    ...
    with patch("httpx.post", side_effect=mock_httpx_post) as mock_post, \
         patch("httpx.get", side_effect=mock_httpx_get) as mock_get:
        
        result = runner.invoke(cli_app, [
            "report", "org/test-repo",
        ...
        mock_post.assert_called_once_with("http://mock-server/api/v1/report/org/test-repo/build", timeout=60.0)
        mock_get.assert_called_once_with("http://mock-server/api/v1/report/org/test-repo/download?format=html", timeout=30.0)
...
def test_cli_report_command_markdown(tmp_path):
    ...
    with patch("httpx.post", side_effect=mock_httpx_post) as mock_post, \
        ...
        mock_post.assert_called_once_with("http://mock-server/api/v1/report/org/test-repo/build", timeout=60.0)
        mock_get.assert_called_once_with("http://mock-server/api/v1/report/org/test-repo/download?format=markdown", timeout=30.0)

...
def temp_store_and_services():
    ...
    arch_svc = ArchitectureService(snapshot_store=store, analysis_cache=cache, graphs_dir=tmpdir)
...
def test_call_graph_service_incremental_updates(temp_store_and_services):
    ...
    context = RepositoryContext(repo_name, repo_path=tmpdir, cache=call_svc.analysis_cache, store=store, graph_service=call_svc.graph_service)
    ...
    # Re-evaluate context
    context_updated = RepositoryContext(repo_name, repo_path=tmpdir, cache=call_svc.analysis_cache, store=store, graph_service=call_svc.graph_service)
    
    # Build Call Graph (Partial)
    gen_partial = call_svc.build_partial(repo_name, changed_files=changed_files, context=context_updated, files=new_files)

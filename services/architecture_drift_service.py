...
class ArchitectureDriftService:
    ...
    def _is_entry_point_file(self, filename: str, imports: List[str]) -> bool:
        ...
        if basename in {"main.py", "__main__.py", "app.py", "application.py", "server.py", "index.js", "server.js", "app.js", "main.tsx", "app.tsx"}:
            return True
        ...
        if "fastapi" in imports_lower or "flask" in imports_lower:
            ...
            if len(parts) <= 2 or basename in {"server.py", "wsgi.py", "asgi.py", "run.py"}:
                return True
    ...
    def analyze_drift(self, owner: str, repo: str, pr_number: int) -> PRDriftResult:
        ...
        for cf in changed_files:
            if cf.status == "added":
                ...
                lang = "python" if ext == ".py" else "javascript" if ext in {".js", ".jsx"} else "typescript" if ext in {".ts", ".tsx"} else "unknown"
                ...
                try:
                    ...
                    if parsed_meta:
                        ...
                        if not G_after.has_node(node_name):
                            G_after.add_node(node_name, language=parsed_meta.get("language", "unknown"), type="file")
                        ...
                        for imp in parsed_meta.get("imports", []):
                            ...
                            if resolved:
                                if not G_after.has_node(resolved):
                                    # Fallback node type
                                    G_after.add_node(resolved, language="unknown", type="file")
                                G_after.add_edge(node_name, resolved, relationship="imports")
                except Exception as e:
                    logger.warning(f"Failed to delta patch file {cf.filename} in graph: {e}")
        ...
        added_dependencies = [DependencyEdge(source=u, target=v) for u, v in added_edges]
        removed_dependencies = [DependencyEdge(source=u, target=v) for u, v in removed_edges]
        ...
        for node in all_nodes:
            ...
            after_val = G_after.in_degree(node) + G_after.out_degree(node) if node in G_after else 0

            if after_val > before_val:
                coupling_increase.append(CouplingChange(file=node, before=before_val, after=after_val))
            elif after_val < before_val:
                coupling_decrease.append(CouplingChange(file=node, before=before_val, after=after_val))
        ...
        max_depth_val = 0
        try:
            pr_res = self.pr_intelligence_service.analyze_pull_request(owner, repo, pr_number)
            blast_radius_lvl = pr_res.blast_radius
            impact_radius_val = pr_res.impact_radius
            max_depth_val = pr_res.max_depth
        except Exception:
            pass

        risk_score, risk_level = self._compute_drift_risk(
            new_cycles=new_cycles,
            changed_core_files=[f for f in baseline_core if any(cf.filename.lower() == f.lower() for cf in changed_files)],
    ...
    def _generate_top_findings(
        ...
        for cyc in new_cycles[:2]:
            ...
            findings.append(f"New dependency cycle introduced: {cycle_str} -> {os.path.basename(cyc[0])}")
        ...
        for c in coupling_increase:
            ...
            if diff >= 5:
                findings.append(f"Coupling increased significantly in {os.path.basename(c.file)} (from {c.before} to {c.after})")

        # 4. Resolved cycles
        if resolved_cycles:
            findings.append(f"Dependency cycle resolved ({len(resolved_cycles)} loops broken)")

        # 5. Coupling decreases
        for c in coupling_decrease[:2]:
            diff = c.before - c.after
            if diff >= 5:
                findings.append(f"Coupling decreased in {os.path.basename(c.file)} (from {c.before} to {c.after})")

        # 6. High blast radius info
        if impact_radius >= 16:
            findings.append(f"High blast radius detected ({impact_radius} downstream files affected)")

        # 7. Added dependencies
        for edge in added_dependencies[:2]:
            findings.append(f"New dependency introduced: {os.path.basename(edge.source)} -> {os.path.basename(edge.target)}")

        # 8. Removed dependencies
        for edge in removed_dependencies[:2]:
            findings.append(f"Dependency removed: {os.path.basename(edge.source)} -> {os.path.basename(edge.target)}")

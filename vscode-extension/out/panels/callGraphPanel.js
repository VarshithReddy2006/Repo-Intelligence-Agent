...
class CallGraphPanel {
    static createOrShow(extensionUri, owner, repo, client) {
        ...
        const panel = vscode.window.createWebviewPanel(CallGraphPanel.viewType, `Call Graph — ${key}`, vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _handleMessage(msg) {
        if (msg.type === 'search') {
            ...
            try {
                const neighbors = await this._client.fetchJson(`/api/call-graph/${this._owner}/${this._repo}/neighbors/${encodeURIComponent(msg.nodeId)}`);
            ...
            try {
                const result = await this._client.getBlastRadius(this._owner, this._repo, msg.nodeId);
...
exports.CallGraphPanel = CallGraphPanel;
CallGraphPanel.viewType = 'repoIntelligenceCallGraph';
CallGraphPanel._panels = new Map();
//# sourceMappingURL=callGraphPanel.js.map
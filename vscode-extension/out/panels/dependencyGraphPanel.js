...
class DependencyGraphPanel {
    static createOrShow(extensionUri, owner, repo, client) {
        ...
        const panel = vscode.window.createWebviewPanel(DependencyGraphPanel.viewType, `Dep Graph — ${key}`, vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _handleMessage(msg) {
        if (msg.type === 'search') {
            ...
            try {
                const trace = await this._client.getGraphTrace(this._owner, this._repo, msg.nodeId);
...
DependencyGraphPanel.viewType = 'repoIntelligenceDepGraph';
DependencyGraphPanel._panels = new Map();
